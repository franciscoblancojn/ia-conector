...
### Nombrado
- **Clases**: `IACON_` prefix, `UPPER_SNAKE` (e.g., `IACON_AI`, `IACON_USE_DATA_CONFIG`).
- **Funciones**: `ia_conector_` prefix, `snake_case`.
- **Constantes**: `IACON_UPPER_SNAKE`.
- **Hooks (actions/filters)**: Prefijo `ia_conector_`.
...
```php
// Generar
wp_nonce_field('ia_conector_action', 'ia_conector_nonce');

// Verificar admin
if (!wp_verify_nonce($_POST['ia_conector_nonce'], 'ia_conector_action')) {
    wp_die('Security check');
}

// Verificar AJAX
check_ajax_referer('ia_conector_nonce', 'nonce');
```
...
- Usa `get_option()`/`update_option()` para opciones (a través de `IACON_USE_DATA_BASE` en este plugin).
- Usa `get_post_meta()`/`update_post_meta()` para meta de posts.

### HTTP API
- Usa `wp_remote_get()`/`wp_remote_post()` en vez de `curl_*` directo.
- Excepción: `IACON_AI::request()` usa cURL directamente para Gemini API.

### AJAX
```php
add_action('wp_ajax_ia_conector_action', 'ia_conector_handle_ajax');
function ia_conector_handle_ajax() {
    check_ajax_referer('ia_conector_nonce', 'nonce');
    if (!current_user_can('edit_posts')) {
